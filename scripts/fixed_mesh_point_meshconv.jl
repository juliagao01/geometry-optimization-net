#!/usr/bin/env julia
# scripts/fixed_mesh_point_meshconv.jl — mesh convergence of f_1 for the SAVED
# optimized shape (design density is mesh-independent; evaluate it on finer
# meshes via exact reg-LU). Lean flags to avoid OOM kills.
...
function f1_at(h)
    geo=joinpath(workdir,"cpm.geo"); inp=joinpath(workdir,"cpm.inp")
    ...
    field = DensityField(cfg; nx=size(bestphys,1), ny=size(bestphys,2), alpha_max=1.0)
...
println("mesh convergence of the optimized shape (exact reg-LU):")
for h in (0.065, 0.05, 0.042, 0.036)
    t=time(); f1,N = f1_at(h)
