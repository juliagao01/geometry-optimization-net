#!/usr/bin/env julia
# scripts/fixed_mesh_tightconv.jl — tighten mesh convergence of f_1 for the saved
# optimized shape (fine 64x40 design grid) across finer solver meshes. Exact reg-LU.
...
@printf("design grid %dx%d\n", DNX, DNY); flush(stdout)

function f1_at(h)
    geo=joinpath(workdir,"tc.geo"); inp=joinpath(workdir,"tc.inp")
    ...
    dot(c, lu(A0+spdiagm(0=>(ALPHA.*rpd).*b1u)+EPS*spdiagm(0=>ones(N)))\(-b)), N
end

println("mesh convergence of optimized shape (exact reg-LU):")
