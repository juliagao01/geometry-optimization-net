...
`0.972` deformed) were transients and are **retracted** — ignore them. The project has
since been rebuilt on a trustworthy footing. Current results (mesh- and
harmonic-converged):

| device / shape | `f_1` |
|---|---|
| obstacle-free vicinity device | ≈ 0.020 |
| round circle obstacle | ≈ 0.033 |
| square obstacle | ≈ 0.094 |
| **optimal deformable oval** (tall channel) | **≈ 0.144** |

Higher-order (nonlinear) coefficients on the optimal circle: `f_2 ≈ 0.088`, `f_3 ≈ −0.0018`.

The enabler was a **structured O-grid** (body-fitted quads wrapping the obstacle) plus a
matrix-free regularized-GMRES solver — this replaced the earlier Brinkman-density and
unstructured-hole attempts, which were α-/h-divergent. See
[`docs/optimal_shape_explained.md`](docs/optimal_shape_explained.md) for the optimal shape
(with figure) and the physics, [`docs/f1_trustworthiness_obstacle_vs_vicinity.md`](docs/f1_trustworthiness_obstacle_vs_vicinity.md)
for the trust audit, and `CLAUDE.md` for the code map. (This work lives on the `nonlinear`
branch; `main` is the clean linear baseline.)
